...
def check(email):
    url = "https://fm247n9sqi.execute-api.us-east-1.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://fm247n9sqi.execute-api.us-east-1.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://fm247n9sqi.execute-api.us-east-1.amazonaws.com/plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://fm247n9sqi.execute-api.us-east-1.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=Z48rb2wkSc7FadWXxYIV3sRy1omCKQ5Hfz0itNUJGlDABvjLun6qQUdic9b4whorYtAasBElzFp5mgZ8&sender_id=FSTSMS&message=Need Plasma of your blood group&language=english&route=p&numbers="+str(i['phone'])
